...
async function crearActividad(datos) {
    ...
    for (let d of datos) {
        ...
        for (let rta of d.respuestas) {
            ...
            inputRta.id = `${rta.txtRespuesta}_${datos.indexOf(d)}_${d.respuestas.indexOf(rta)}`;
            inputRta.value = rta.correcta;
            
            let labelRta = document.createElement('label');
            labelRta.setAttribute('for', `${rta.txtRespuesta}_${datos.indexOf(d)}_${d.respuestas.indexOf(rta)}`);
